...
import {inputchange,actionclick_c,actionclick_eval,change_input_e,actionsClick,numClick} from './store/actionCreators';
...
export default class Calculator extends React.Component {
  ...
  actionsClick(e){
    ...
    if("=" === e){
      try{
        let value = eval(this.state.evalStr);
        const action = actionclick_eval(value);
        store.dispatch(action);
      }catch(e){
        //this.state.result = "";
        this.error = this.errorMsg;
    }
      // if (false === /^\d+(\.\d+)?$/.test(this.state.result)) {
      //    //this.state.result = "";
